...
TASK_TMPL = (
    ...
    "structure; (2) find and record the project's {label} from the packaging/config files; "
    "(3) inspect AT LEAST 8 different source modules spread across the tree — for each, "
    "note its responsibility and one notable function/class with a one-line excerpt; "
    "(4) trace one cross-module dependency chain. Finish with the guide: a component map "
    "plus 3 handover bullets, one of which states the {label} exactly as found. Use at "
    "least 14 tool calls; do not paraphrase file excerpts."
)
...
def run_agent(work, label, timeout=900):
    ...
    task = TASK_TMPL.format(label=label)
...
def main():
    ...
    label, value, path = facts[0]
    print(f"fact: {label}={value!r} (from {path}); running pi agent in {work}")

    transcript = run_agent(work, label)
    n_occ = transcript.count(value)
    print(f"transcript: {len(transcript):,} chars | fact occurrences: {n_occ}")
    if n_occ < 1:
        raise SystemExit("agent never observed the fact — discard (regenerate with a "
                         "different repo/fact)")
    ...
    with open(args.out, "w") as f:
        ...
        q = (f"Session transcript follows.\n\n{transcript}\n\nQuestion: according to the "
             f"session above, what is the project's {label}? Reply with the value only.")
        f.write(json.dumps({"prompt": q, "answer": value,
                            "question": f"{label}?", "axis": "agentmem"}) + "\n")
    print(f"wrote 2 lines -> {args.out} (validate with serialize.py)")
    if args.validate:
        assert value in transcript
        print("agent_traj validation OK (fact grounded, exactly observed)")
